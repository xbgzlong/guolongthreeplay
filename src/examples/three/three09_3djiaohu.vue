	...
	methods: {
        ...
        setCube(){
            ...
            const params = {
                ...
                radius:7,//半径
                branch:6,//分支数量
                centerColor:"#ff0000",//中心颜色
                siper:0.5,//弯曲程度
                sideColor:"#ffffff",//边缘颜色
                
            }
            let  centerColor = new THREE.Color(params.centerColor)
            let  sideColor = new THREE.Color(params.sideColor)
            let material =null;
            ...
            const makeGeometry=()=>{
                ...
                for(let i = 0; i<params.count;i++){
                    let current  = i*3;
                    //获取各个分支之间的角度angel
                    let angel =  (i%params.branch)*(2*Math.PI/params.branch);
                    // 当前点距离圆心的距离
                    let distance = Math.pow(Math.random(),3) * params.radius;
                    let randomX = Math.pow(Math.random()*2-1,3) *(params.radius-distance) /5;
                    let randomY = Math.pow(Math.random()*2-1,5) *(params.radius-distance) /5;
                    let randomZ = Math.pow(Math.random()*2-1,3) *(params.radius-distance) /5;
                    // 给每一个X轴赋值为随机的值
                    positionARR[current] = Math.cos(angel + distance*params.siper )*distance + randomX;
                    // 给每一个X轴赋值为0
                    positionARR[current+1] = randomY;
                    // 给每一个z轴赋值为随机数
                    positionARR[current+2] = Math.sin(angel + distance *params.siper) *distance + randomZ;
                    // colorARR[current] = Math.random();
                    // colorARR[current + 1] = Math.random();
                    // colorARR[current + 2] = Math.random();
                    let mixColor = centerColor.clone();
                    // 颜色渐变  distance/params.radius颜色渐变度0-1
                    mixColor.lerp(sideColor, distance/params.radius)
                ...
                this.boxGeometry.setAttribute(
                    'position',new THREE.BufferAttribute(positionARR,3)
                )
                // 设置材质颜色 
                this.boxGeometry.setAttribute(
                    'color',new THREE.BufferAttribute(colorARR,3)
                )
                 // 增加一个点材质  
                material = new THREE.PointsMaterial( 
                    {
                        color: 0xffffff,
                        size:params.size,// 大小
                        sizeAttenuation:true,// 根据距离衰减大小
                        map:mappic,// 设置材质的纹理贴图
                        alphaMap:mappic, // 透视贴图 黑色变透明
                        transparent:true, // 透明
                        depthwrite:false, // 渲染后面被遮挡的内容
                        blending:THREE.AdditiveBlending,// 显示重合效果
                        vertexColors:true,//采用图形的颜色
                    } 
                ) 
            }
            makeGeometry();

            this.cube = new THREE.Points(this.boxGeometry,material);
        ...
        setGsap(){
          ...
          //  动画渲染 
        setRender(time){
            this.cube.rotation.y = time*0.0003;
