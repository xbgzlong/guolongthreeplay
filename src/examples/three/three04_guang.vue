...
export default {
	...
	methods: {
        ...
        setCube(){
            ...
            // 设置贴图 有回调函数可以查看加载进度，结果等等 onLoad : Function, onProgress : Function, onError : Function
            const mappic = TextureLoader.load(require('../../assets/images/TilesMosaicPennyround001_GLOSS_2K.png'),function(){},function(){},function(){},);
            ...
            let material = new THREE.MeshStandardMaterial( {color: 0xffffff,map:mappic,} ) 
            this.cube = new THREE.Mesh(this.boxGeometry,material);
            this.scene.add(this.cube);
            // 立方体
            const BoxGeometry = new THREE.BoxGeometry( 3, 3, 3, 150, 150, 150 ); 
            
            // 纹理偏移
            // mappic.offset.set(0.5,0.5)
            // 设置旋转中心点
            // mappic.center.set(0.5,0.5) ;
            // 纹理旋转45度
            // mappic.rotation = Math.PI/4;
            // 贴图重复 水平贴2 ，垂直3
            // mappic.repeat.set(2,3);
            // 水平镜像拍了
            // mappic.wrapS = THREE.MirroredRepeatWrapping ;
            // 垂直无限复制
            // mappic.wrapT =  THREE.RepeatWrapping;
            // 最接近像素效果的纹理设置
            // mappic.minFilter = THREE.NearestFilter;
            // mappic.magFilter = THREE.NearestFilter;
            // 默认是纹理线性模糊效果
            // mappic.minFilter = THREE.LinearFilter;
            // mappic.magFilter = THREE.LinearFilter;
            // 设置贴图
            const othermappic = TextureLoader.load(require('../../assets/images/TilesMosaicPennyround001_DISP_2K.png'));
            const displacementMap = TextureLoader.load(require('../../assets/images/TilesMosaicPennyround001_BUMP16_2K.png'));
            const roughnessMap = TextureLoader.load(require('../../assets/images/TilesMosaicPennyround001_NRM_2K.png'));
            const BoxGeometrycube = new THREE.Mesh( BoxGeometry, new THREE.MeshStandardMaterial( 
                {
                    // color: 0xffffff,
                    // 颜色贴图
                    map:mappic,
                    // 透明贴图
                    // alphaMap:othermappic,
                    // transparent:true,
                    // 位移贴图，需要设置各个轴的位移量
                    ...
                    // 粗糙度0.0表示平滑的镜面反射，1.0表示完全漫反射。默认值为1.0。如果还提供roughnessMap，则两个值相乘。
                    roughness:0.9,
                } ) ); 
            BoxGeometrycube.position.set(0,0,0);
            // 添加灯光
            // 从上方照射的白色平行光，强度为 0.9。
            ...
            directionalLight2.target = this.cube;
